...
class MainWin(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def initUi(self):
        ...
        # 测试仪响应定时器
        self.devResponseTimer = QTimer()# 使用定时器，防止主界面卡死
        self.devResponseTimer.timeout.connect(self.deviceNoResponse)
        # 接通电源参数下发定时器
        self.powerOnParaTimer = QTimer() # 使用定时器，防止主界面卡死
        self.powerOnParaTimer.timeout.connect(self.autoSendParameters)
        # 参数下发阈值
        self.startParaTimer = QTimer() # 使用定时器，防止主界面卡死
        ...
        self.enableFuncBtnTimer = QTimer()
        self.enableFuncBtnTimer.timeout.connect(self.enableFuncAction)
    ...
    def enableFuncAction(self):
        ...
        if self.enableFuncBtnTimer.isActive():
            self.enableFuncBtnTimer.stop()

    def disableFuncAction(self):
        ...
        if l != 0:
            if self.confirmDetection:
                ...
                if choice == QMessageBox.Yes:
                    ...
                    self.tableviewRow = self.tableviewRow - l
                    self.pushBtn_saveResults.setEnabled(False)
                    self.pushBtn_saveResultsAs.setEnabled(False)
...
#------------------START----------------------按钮槽函数------------------START----------------------#
    ...
    def on_pushBtn_deviceDetection_clicked(self):
        self.enableFuncBtnTimer.start(60*1000)
...
#------------------END----------------------按钮槽函数------------------END----------------------#
    ...
    def parseQueryCodeResults(self):
        ...
        if self.queryCode == 'NODET':
            ...
            if self.isPureQueryCode:
                ...
                pass
        if self.encDetEncdetQuery == 3 and self.isPureQueryCode == True:
            self.enableFuncAction()
        self.isDeviceResponsed = True
        self.isPureQueryCode = None
        self.flushSerialBuffer()
    ...
    def lineVoltageCurrentCheck(self, rxData, codeCheckOK):
        l = len(rxData)
    ...
    def markAndShowTheResults(self, col):
        if col != 14:
        ...
        item.setFont(QFont('Times New Roman', 16, QFont.Weight.Light))
        return item
